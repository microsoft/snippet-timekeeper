...
public final class Snippet {
     ...
     */
    public static ExecutionContext capture(String message, Closure closure) {
        return EXECUTION_PATH.get().capture(message, closure);
    }
     ...
     * @param closure Lambda or implementation representing the closure.
     */
    public static ExecutionContext capture(Closure closure) {
        return EXECUTION_PATH.get().capture(null, closure);
    }
    ...
    public static class LogToken implements ILogToken {
        ...
        public void addSplit() {
            ...
            long splitInMillis = getSplitInMillis();
            mSplitCount++;
            Log.d(mFilter, String.format(SPLIT_MESSAGE, mSplitCount, splitInMillis));
        }
        ...
        public void addSplit(String message) {
            ...
            long splitInMillis = getSplitInMillis();
            mSplitCount++;
            Log.d(mFilter, String.format(SPLIT_MESSAGE, "[" + mSplitCount + "]" + message, splitInMillis));
        }

        private long getSplitInMillis() {
            long delta;
            if (mLastSplitTimeCaptured == 0L) {  // Split called for the first time
                ...
                delta = mLastSplitTimeCaptured - getStart();
            } else {
                // Here we use the last split time captured.
                long temp = ToolBox.currentTime();
                delta = temp - mLastSplitTimeCaptured;
                mLastSplitTimeCaptured = temp;
            }
            return delta;
        }
        ...
        public ExecutionContext endCapture() {
            ...
            if (ToolBox.willThreadLockGuardThisCapture(Thread.currentThread(), this)) {
                ...
                throw new UnsupportedOperationException("endCapture() should always be called on "
                        + "the same thread that created log token with Snippet.startCapture() while "
                        + "thread locks are enabled");
            } else {
